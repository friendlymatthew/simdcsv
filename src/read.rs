use crate::classify::CsvClassifier;
use crate::classify::{COMMA_CLASS, NEW_LINE_CLASS, QUOTATION_CLASS};
...
pub fn read(data: &[u8]) -> Vec<RowRef> {
    ...
    for i in 0..quotation_bitsets.len() {
        ...
        let mut valid_commas = comma_bitsets[i] & outside_quotations;
        let mut valid_new_line = new_line_bitsets[i] & outside_quotations;
...
#[inline]
const fn _mark_inside_quotations(mut x: u64, carry: bool) -> u64 {
    x ^= x >> 1;
    x ^= x >> 2;
    x ^= x >> 4;
    x ^= x >> 8;
...
mod tests {
    ...
    fn read_taxi_zone_lookup() {
        let data = std::fs::read_to_string("taxi_zone_lookup.csv").unwrap();

        let mut csv = Vec::new();

        for row in read(data.as_bytes()) {
            let fields = row
                .iter()
                .map(|field_range| {
        ...
        insta::assert_debug_snapshot!(csv, @r#"
        [
            [
                "\"LocationID\"",
                "\"Borough\"",
